...
try:
    ...
    def _on_collision(data):
        print('Collision')
        actor_type= gadn(data.other_actor)
        print('Collision Happened Between :- ' ,actor_type)
        collision_event_record= data.normal_impulse
        ioc=math.sqrt(collision_event_record.x **2 + collision_event_record.y **2 + collision_event_record.z **2)
        print('Intensity Of The Collision Was :- ' ,ioc)
        dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
        time.sleep(5)
