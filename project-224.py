...
def Bike_control():
    ...
    dropped_vehicle.apply_control(carla.VehicleControl(steer=-0.35, throttle=0.25))
    time.sleep(2)

    # Line 61: Turn on the bike lights (HighBeam, Brake lights, and LeftBlinker)
    dropped_vehicle.set_light_state(carla.VehicleLightState(
        carla.VehicleLightState.HighBeam | carla.VehicleLightState.Brake | carla.VehicleLightState.LeftBlinker))

    # Line 63: Drive the Bike to the left for 2 milliseconds
    time.sleep(2)

    # Line 64: Slow down the speed of the bike
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.2))
    
    # Line 65: Turn on the bike lights (HighBeam and Brake lights)
    dropped_vehicle.set_light_state(carla.VehicleLightState(
        carla.VehicleLightState.HighBeam | carla.VehicleLightState.Brake))

    # Line 67: Drive the Bike to the left for 3 milliseconds
    time.sleep(3)

    # Line 68: Stop the bike using hand_brake
    dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
