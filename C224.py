...
def generate_radar_blueprint(blueprint_library):
    ...
    radar_blueprint.set_attribute('points_per_second', str(56000))
...
def _Radar_callback(radar_data):
    ...
    for detection in radar_data:
        ...
        if distance_name_data['distance'] >5 and distance_name_data['distance']<7:
            print('Slowing down the Vehicle')
            dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.31))
            if distance_name_data['distance'] >5 and distance_name_data['distance']<7:
                dropped_vehicle.apply_control(carla.VehicleControl(brakes=True))
                print('Distance and the Name Data are :- ', distance_name_data)
            break
        else:
